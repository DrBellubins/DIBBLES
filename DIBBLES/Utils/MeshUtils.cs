...
public static class TextureUtils
{
    ...
    public static Texture2D GetWhitePixel()
    {
        if (whitePixel == null)
        {
            whitePixel = new Texture2D(Engine.Graphics, 1, 1);
...
public static class MeshUtils
{
    ...
    public static CubeMesh GenTexturedCube(GraphicsDevice graphicsDevice, Texture2D texture)
        => new CubeMesh(graphicsDevice, texture);

    // Sets the texture on an existing cube mesh
    public static void SetCubeMeshTexture(CubeMesh mesh, Texture2D texture)
