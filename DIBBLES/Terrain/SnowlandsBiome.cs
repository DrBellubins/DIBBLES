...
public class SnowlandsBiome
{
    public void Generate(ref BlockReturnData bRetData)
    {
        ...
        {
            // This is the surface
            returnData.CurrentBlock = new BlockData(returnData.WorldPos, Block.Prefabs[BlockType.Snow]);
            returnData.FoundSurface = true;
            returnData.IslandDepth = 0;
        }
        else if (returnData.IslandDepth < 3) // snow thickness = 4
        {
            returnData.CurrentBlock = new BlockData(returnData.WorldPos, Block.Prefabs[BlockType.Snow]); // TODO: Should be ice
